...
class MeowSettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tvDefaultCount = TextView(this).apply { textSize = 18f; setTypeface(typeface, android.graphics.Typeface.BOLD) }
        val tvAddedCount = TextView(this).apply { textSize = 18f; setTypeface(typeface, android.graphics.Typeface.BOLD) }
        ...
        fun refreshCountsAndToday() {
            ...
            val source = pref.getString(KEY_SOURCE, "all") ?: "all"
            val favs = (pref.getString(KEY_FAVS, "") ?: "").split("\n").filter { it.isNotEmpty() }
            val allAll = (defaults + added).distinct()
            val list = if (source == "fav") favs else allAll
            if (list.isEmpty()) {
        ...
        btnPaste.setOnClickListener { openPasteDialog { added -> toast("Đã thêm: $added"); refreshCountsAndToday() } }
    ...
    private fun ensurePlanBase(size: Int): Int {
        ...
        var base = max(0, pref.getInt(KEY_PLAN_IDX, -1))
        if (oldDay == null) base = 0
        else if (oldDay != today) base = (base + 1) % max(1, size)
