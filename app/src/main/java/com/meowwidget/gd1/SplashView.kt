...
class SplashView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
  ...
  // Grid & anchors (C,R) as bạn đã chốt (áp dụng cho cả 3 wave)
  ...
  override fun onDraw(canvas: Canvas) {
    ...
    // "Cover" crop từ ảnh gốc để nền blur không có viền đen
    ...
    // Tính khung ảnh theo tỉ lệ thật (letterbox) để biết mép dưới
    ...
    // Extra blur chỉ ở phần dưới (giảm ~10% vs 60f)
    ...
    // Mask để GIỮ blur mạnh ở dưới (từ contentBottom -> đáy), mượt dần
    val maskPaint = Paint().apply {
      xfermode = PorterDuffXfermode(PorterDuff.Mode.DST_IN)
      shader = LinearGradient(
        0f, contentBottom.toFloat(),
        0f, height.toFloat(),
        0x00000000.toInt(),    // trong suốt tại mép ảnh
    ...
    for (w in 0 until 3) {
      ...
      for (i in coords.indices) {
        ...
        // Dải dừng riêng cho từng cánh (ổn định)
    ...
    // Khung bảng theo toạ độ phần trăm đã chốt: x:30..70, y:60..73 (% màn)
