...
content.addView(actionRow)
    ...
    val rootVG = root as ViewGroup

    // Divider mảnh 1dp, mờ (alpha ~25%), cách phần trên 32dp
    val divider = View(this).apply {
        setBackgroundColor(0x40000000) // #40000000 (đen 25%)
    }
    rootVG.addView(divider, ViewGroup.MarginLayoutParams(
        ViewGroup.LayoutParams.MATCH_PARENT, dp(1f)
    ).apply { topMargin = dp(32f) })

    // Footer container: dọc, canh giữa, padding top/bottom
    val footer = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
        gravity = Gravity.CENTER_HORIZONTAL
        setPadding(0, dp(12f), 0, dp(24f))
    }
    rootVG.addView(footer, ViewGroup.MarginLayoutParams(
        ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT
    ))

    fun TextView.sz(sp: Float) = setTextSize(TypedValue.COMPLEX_UNIT_SP, sp)

    // 1) The Meow Widget (đậm nhẹ)
    TextView(this).apply {
    ...
    // 4) v1.0 · © 2025  (có khoảng cách nhỏ phía dưới)
    TextView(this).apply {
        text = "v1.0 · © 2025"
        sz(12f)
